...
class Payment(models.Model):
    ...
    pay_amount = models.DecimalField(decimal_places=2)
    pay_escro_user = models.CharField(max_length=20, null=True) # 예금주
    pay_escro = models.CharField(max_length=20, null=True)
    user = models.ForeignKey(User, on_delete=models.SET_NULL, null=True) # user 객체 삭제되면 user=null, 카드정보로 찾기? (*카드번호 필드 부재)
...
class Settlement(models.Model):
    ...
    settlement_amount = models.DecimalField(decimal_places=2)
    set_account = models.IntegerField(max_length=14, null=True) # 계좌번호
    ...
    seller = models.ForeignKey(Seller, on_delete=models.SET_NULL, null=True) # user 객체 삭제 시 계좌정보로 정산정보 조회
